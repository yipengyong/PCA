...
train_index = list(np.loadtxt('train_index.txt', dtype=np.float))
for i in range(len(train_index)):
    train_index[i] = int(train_index[i])
...
# 训练
parameters = nn_model(X, Y, n_h1=30, n_h2=30, num_iterations=10000, learning_rate=0.2)
np.savetxt('w1.txt', parameters['w1'], fmt="%0.8f")
np.savetxt('w2.txt', parameters['w2'], fmt="%0.8f")
np.savetxt('b1.txt', parameters['b1'], fmt="%0.8f")
np.savetxt('b2.txt', parameters['b2'], fmt="%0.8f")
...
for i in range(len(Y_pred[0])):
    if Y_pred[0][i] != Y_test[0][i]:
        wrongnum += 1
print('错误率', wrongnum/len(X_test[0]))
